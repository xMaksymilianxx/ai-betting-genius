# api/analyze.py - WERSJA Z OSTATECZNYM TRYBEM DIAGNOSTYCZNYM (LOGI W ODPOWIEDZI)
import requests
import json
import time
...
def fetch_sportmonks_live() -> List[Dict]:
    ...
    try:
        ...
        print(f"  [LOG] Odpowiedź API (fragment): {response.text[:400]}")
...
# ==================== GŁÓWNY HANDLER VERCEL (Z PRZECHWYTYWANIEM LOGÓW) ====================
from http.server import BaseHTTPRequestHandler
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        response_data = {}
        status_code = 500
        # Przechwyć wszystko, co zostanie wypisane przez print()
        ...
        with redirect_stdout(log_stream):
            try:
                ...
                if not matches_data['matches']:
                    status_code = 200
                    response_data = {'success': False, 'message': 'DIAGNOSTYKA: Nie znaleziono meczów na żywo w żadnym z API.', 'matches_found': 0}
                else:
                ...
                response_data = {'success': False, 'error': str(e), 'traceback': traceback.format_exc().splitlines()}
                status_code = 500
        
        # Dodaj przechwycone logi do finalnej odpowiedzi
        diagnostic_logs = log_stream.getvalue().splitlines()
